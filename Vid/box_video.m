...
function annot_video = box_video(video, annotations, color)
    ...
    for f=1:frames
        ...
        % draw box
        a = annotations{f};
        cdata(a.ytl:a.ybr, a.xtl, :) = repmat(color, [a.ybr-a.ytl+1 1 1]);
        cdata(a.ytl:a.ybr, a.xbr, :) = repmat(color, [a.ybr-a.ytl+1 1 1]);
        cdata(a.ytl, a.xtl:a.xbr, :) = repmat(color, [a.xbr-a.xtl+1 1 1]);
        cdata(a.ybr, a.xtl:a.xbr, :) = repmat(color, [a.xbr-a.xtl+1 1 1]);
        % save frame
